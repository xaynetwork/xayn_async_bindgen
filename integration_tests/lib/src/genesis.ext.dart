...
class Api2 {
    ...
    Api2(this._inner) {
        final status = _inner.async_bindgen_dart_init_api__api2(NativeApi.initializeApiDLData);
        if (status != 1) {
...
class AsyncApi {
    ...
    AsyncApi(this._inner) {
        final status = _inner.async_bindgen_dart_init_api__async_api(NativeApi.initializeApiDLData);
        if (status != 1) {
            throw CouldNotInitializeDartApiError();
        }
    }
